...
- `src/client/shared.ts`: Shared `renderMarkdown()` parser, theme switcher, WebCrypto AES-256-GCM / PBKDF2 crypto, sanitization.
...
- `src/styles.ts`: Paper & Ink dual-theme CSS tokens (`CSS_VARIABLES`) and component stylesheets.
- `test/index.test.ts`: Bun unit test suite (48 tests).
...
- **Submit Double-Firing:** Rapid Ctrl+S during PBKDF2/network created duplicate pastes. Fix: Guard submission with `isSubmitting` flag.
